$(document).ready(function(){
  window.addEventListener('message', function( event ) {      
    if (event.data.action == true) {
      ...
      $('#stock').empty();
      $('#stock').append("<b>Carburant en stock:<BR>" + dataGas.stock + " Litres</b>");
  ...
  $( "#retirar" ).click(function() {
    if (!counting) {
      ...
      completar = !completar;
      maxFuel = 99
      $('.modal span').text("Voulez-vous remplir le réservoir à $"+(perc_new*dataGas.price).toFixed(2));  
      $('.modal').fadeIn(300);
      $('.container').fadeOut();
      $('left').fadeOut();
      $('.act').fadeOut();
    ...
    if (!counting) {
      ...
      if (price > dataGas.price) {  
        if (price <= ((100-Math.round(datafuel))*dataGas.price)) {
          ...
          $('.modal span').text("Voulez-vous remplir le réservoir à $ "+price); 
          $('.modal').fadeIn(300);
          $('.container').fadeOut();
          $('left').fadeOut();
          $('.act').fadeOut();
        } else {
          var texto = "La valeur maximale pour remplir le réservoir est <b>$"+((100-Math.round(datafuel))*dataGas.price)+"</b>!"
          $.post('http://esx_fuel/notifytext', JSON.stringify({ text: texto }));
        }
      } else {
        var texto = "Le prix doit être supérieur à $" + dataGas.price + "!"
        $.post('http://esx_fuel/notifytext', JSON.stringify({ text: texto }));
      }
    }
  });
